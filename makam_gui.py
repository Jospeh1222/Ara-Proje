...
class PredictionThread(QThread):
    ...
    def __init__(self, segment_images, entry, parent=None):
        super().__init__(parent)
        self.segment_images = segment_images   # raw uint8 list
        self.entry = entry                     # tek bir ModelEntry

    def run(self):
        try:
            entry = self.entry
            self.progress_updated.emit(0, 1)
            self.status_updated.emit(f"Model yükleniyor: {entry.display_name}")
            ...
            if not entry.is_available:
                ...
                self.finished_success.emit([(entry, None)])
                return

            self.status_updated.emit(f"Tahmin yapılıyor: {entry.display_name}")
            try:
...
class MakamGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        # Sadece tek bir .parent kullanıyoruz
        base_dir = Path(__file__).resolve().parent
        self.registry = pred.build_model_registry(base_dir / "experiments")
    ...
    def _build_prediction_panel(self) -> QWidget:
        ...
        subtitle = QLabel("Bir model seçin ve tahmini başlatın")
        subtitle.setObjectName("subtitle")
        ...
        outer_layout.addWidget(selector_label)

        outer_layout.addSpacing(6)

        self.model_combo = QComboBox()
        for entry in self.registry:
            self.model_combo.addItem(entry.display_name)
        outer_layout.addWidget(self.model_combo)

        outer_layout.addSpacing(8)

        self.model_desc_label = QLabel("")
        self.model_desc_label.setObjectName("modelDesc")
        self.model_desc_label.setWordWrap(True)
        outer_layout.addWidget(self.model_desc_label)

        # İlk açıklamayı göster (default = ilk model = 20 makamlık CNN)
        if self.registry:
            self.model_desc_label.setText(self.registry[0].description)
    ...
    def _on_extraction_done(self, segments: list):
        """Ozellik cikarma bitti, secili model ile tahmin baslat"""
        self.segment_features = segments

        secili_idx = self.model_combo.currentIndex()
        secili_entry = self.registry[secili_idx]

        self.status_label.setText(
            f"{len(segments)} segment hazır, model çalışıyor"
        )
        self.extraction_progress.setValue(0)
        self.progress_text.setText("Model yükleniyor")

        self.prediction_thread = PredictionThread(segments, secili_entry)
    ...
    def _on_model_changed(self, index: int):
        """Dropdown'dan model seçilince açıklamayı güncelle"""
        if 0 <= index < len(self.registry):
            self.model_desc_label.setText(self.registry[index].description)
    ...
    def _show_placeholder(self, text: str = None):
        if text is None:
            text = (
                ...
                "Yukarıdan bir model seçebilirsiniz.\n"
                "Seçilen modelin tahmini burada gösterilecek."
            )
        lbl = QLabel(text)
