%%writefile /content/make_kfolds_spec.py

# PNG spektrogram segmentleri için K-fold üretir
# make_kfolds.py ile aynı mantık, fark: SPEC_CACHE_DIR ve .png uzantısı
# Aynı şarkı ID'leri kullanılır — NPZ ve PNG fold'ları karşılaştırılabilir
...
def main():
    ...
    # SPEC_CACHE_DIR altındaki PNG dosyalarını tara
    by_class = defaultdict(lambda: defaultdict(list))
    for cls_dir in sorted(p for p in C.SPEC_CACHE_DIR.iterdir() if p.is_dir()):
    ...
    # Aynı SEED kullanılıyor — NPZ fold'larıyla aynı şarkı bölünmesi
    test_assignments = stratified_kfold_song_ids(songs_per_class, C.N_FOLDS, C.SEED)

    folds = []

    print(f"\n{C.N_FOLDS}-fold stratifiye bolunme — SPEKTROGRAM PNG")
    ...
    with open(C.FOLDS_SPEC_FILE, "w", encoding="utf-8") as f:
        json.dump(out, f, indent=2, ensure_ascii=False)
    print(f"\nKaydedildi -> {C.FOLDS_SPEC_FILE}")
