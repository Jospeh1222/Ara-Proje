...
import matplotlib
matplotlib.use("Agg")  # ekransız ortam için (Colab uyumlu)
...
# PNG boyutu — pretrained modeller genelde 224x224 veya 256x256 bekler
IMG_SIZE = 224  # piksel (kare çıktı)


def save_spectrogram_png(y: np.ndarray, sr: int, out_path: Path):
    """STFT magnitude spektrogram hesapla ve viridis colormap ile PNG kaydet."""
    stft   = np.abs(librosa.stft(y, n_fft=C.N_FFT, hop_length=C.HOP_LENGTH))
    ...
    librosa.display.specshow(
        ...
        x_axis=None,   # eksen etiketleri yok, sadece görüntü
        y_axis=None,
        cmap="viridis",
        ax=ax,
    )
    ax.set_axis_off()
    fig.subplots_adjust(left=0, right=1, top=1, bottom=0)  # kenar boşluğu sıfırla
...
def process_file(audio_path: Path, out_dir: Path) -> int:
    ...
    for i, start in enumerate(range(0, len(y) - SEG_SAMPLES + 1, HOP_SAMPLES)):
        ...
        if out_path.exists():   # zaten işlendiyse atla
            written += 1
            continue

        seg = y[start:start + SEG_SAMPLES]
        save_spectrogram_png(seg, sr, out_path)
        written += 1
...
def main():
    ...
    print(f"PNG boyutu: {IMG_SIZE}x{IMG_SIZE} | Colormap: viridis\n")
    ...
    for cls_dir in classes:
        ...
        # Mel cache'inden AYRI klasöre kaydet
        out_cls = C.SPEC_CACHE_DIR / cls_dir.name
