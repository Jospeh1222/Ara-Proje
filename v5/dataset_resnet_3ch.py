...
class MakamResNet3chDataset(Dataset):
    ...
    def __init__(self, fold: int, split: str = "train", augment: bool = False):
        ...
        #ImageNet stats - pretrained ResNet bu degerlerle egitildi
        self.mean = np.array(C.IMAGENET_MEAN, dtype=np.float32)
        self.std  = np.array(C.IMAGENET_STD,  dtype=np.float32)
