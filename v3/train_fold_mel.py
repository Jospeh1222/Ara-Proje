#Mel PNG icin tek fold egitim

#Kullanim:
#python train_fold_mel.py 0      -> sadece fold 0
#python train_fold_mel.py all    -> tum foldlar
...
import config as C
from dataset_mel import MakamMelPNGDataset
...
def safe_save(src_path, dst_path):
    if not drive_available():
        return False
    try:
        os.makedirs(os.path.dirname(str(dst_path)), exist_ok=True)
...
def train_one_fold(fold: int):
    ...
    print(f"\n{'#' * 60}")
    print(f"# MEL-PNG FOLD {fold} / {C.N_FOLDS - 1}")
    print(f"{'#' * 60}")
    print(f"Cihaz: {device}")

    if not C.stats_mel_path(fold).exists():
        raise SystemExit(f"Stats yok: {C.stats_mel_path(fold)}. "
                         f"Once compute_stats_mel.py calistir.")

    train_ds = MakamMelPNGDataset(fold, "train", augment=True)
    val_ds   = MakamMelPNGDataset(fold, "val",   augment=False)
    test_ds  = MakamMelPNGDataset(fold, "test",  augment=False)
    ...
    best_path = C.ckpt_best_path(fold)

    #Erken durdurma degiskenleri
    start_epoch       = 1
    ...
    for epoch in range(start_epoch, C.MAX_EPOCHS + 1):
        ...
        #Her epoch sonu last.pt'yi guncelle (Drive'a yaziyoruz - bu zaten Drive)
        torch.save({
    ...
    print_report(seg_metrics, idx_to_label, f"MEL-PNG Fold {fold} - Segment")
    ...
    print_report(song_metrics, idx_to_label, f"MEL-PNG Fold {fold} - Sarki")
    ...
    out_path = C.fold_results_path(fold)
