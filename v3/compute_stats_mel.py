#Mel PNG segmentleri icin fold bazinda normalizasyon istatistikleri
#Sadece o foldun TRAIN setini kullanir
...
def compute_for_fold(fold: int):
    ...
    if not C.FOLDS_MEL_FILE.exists():
        raise FileNotFoundError(f"folds dosyasi yok: {C.FOLDS_MEL_FILE}. "
                                f"Once make_kfolds_mel.py calistir.")
    ...
    #Per-kanal (R, G, B) toplam ve kareler toplami
    ...
    with open(C.stats_mel_path(fold), "w") as f:
        json.dump(out, f, indent=2)
    print(f"  Kaydedildi -> {C.stats_mel_path(fold)}")
...
def main():
    ...
    if run_mode == "all":
        for k in range(C.N_FOLDS):
            if C.stats_mel_path(k).exists():
                print(f"\n=== Fold {k} zaten mevcut, atlaniyor ===")
                continue
            print(f"\n=== Fold {k} ===")
            compute_for_fold(k)
    else:
