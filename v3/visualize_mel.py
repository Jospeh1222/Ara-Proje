...
def plot_confusion_matrix(results, classes, out_path, level="song_level"):
    ...
    ax.set_title(f"Karisiklik Matrisi - {title_tr} (Mel PNG, 5 fold)",
                 fontsize=13, pad=15)
...
def plot_per_class_f1(results, classes, out_path, level="song_level"):
    ...
    title_tr = "Sarki Seviyesi" if level == "song_level" else "Segment Seviyesi"
    ax.set_title(f"Sinif Basina F1 Skorlari - {title_tr} (Mel PNG)",
                 fontsize=13, pad=15)

    ax.axhline(y=0.50, color="gray", linestyle="--", linewidth=0.8, alpha=0.5)
    ax.axhline(y=0.35, color="gray", linestyle="--", linewidth=0.8, alpha=0.5)
...
def plot_fold_consistency(results, out_path, level="song_level"):
    ...
    ax.set_ylim(0, max(0.5, max(max(per_fold[k]) for k in metric_keys) + 0.05))

    title_tr = "Sarki Seviyesi" if level == "song_level" else "Segment Seviyesi"
    ax.set_title(f"Foldlar Arasi Metrik Tutarliligi - {title_tr} (Mel PNG)",
                 fontsize=13, pad=15)
...
def main():
    ...
    fig_dir = C.RESULTS_DIR / "figures_mel"
